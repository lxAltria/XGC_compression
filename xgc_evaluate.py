...
def compute_diff(name, x, x_):
	...
	print("{}, shape = {}: relative L-inf error = {}, rmse = {}".format(name, x.shape, max_diff/vrange, rmse))

# compare
qoi = np.load("qoi.npz")
print("")
compute_diff("density", density, qoi['arr_0'])
compute_diff("u_para", u_para, qoi['arr_1'])
compute_diff("T_perp", T_perp, qoi['arr_2'])
compute_diff("T_para", T_para, qoi['arr_3'])
compute_diff("n0_avg", n0_avg, qoi['arr_4'])
compute_diff("T0_avg", T0_avg, qoi['arr_5'])
